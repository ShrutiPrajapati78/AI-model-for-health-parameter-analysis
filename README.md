...
- 🔐 **Login system** — multi-user (username/password), powered by `streamlit-authenticator`
...
**To add/change users:** edit `config.yaml` — add a new entry under `credentials: usernames:` with `email`, `name`, and a plain-text `password` (it gets hashed automatically at runtime, you don't need to hash it yourself).

⚠️ **Security note:** This is a simple demo-grade auth suitable for a college/internship presentation — passwords live in a local YAML file. It is **not** production-grade security (no password reset, no encryption at rest beyond hashing, no rate-limiting). Good enough to show "the app requires login" in a demo; not something to expose on the public internet with real patient data.
